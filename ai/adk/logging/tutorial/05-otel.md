...
# note: a LIST of hooks, and a project-scoped resource so the telemetry
# endpoint can route the spans (without it the export is rejected with a 400).
...
Spans land in **Cloud Trace**; GenAI events land in **Cloud Logging** under
per-event log names `gen_ai.*` (for example `gen_ai.user.message`,
`gen_ai.choice`, `gen_ai.system.message`). This mode needs two extra packages, already in
`requirements.txt`: `opentelemetry-exporter-otlp-proto-http` and
`opentelemetry-exporter-gcp-logging`.

---

### 5.1 The privacy knob you must know about

The GenAI **content events** carry prompt and response text, and one environment
variable controls whether they do. It defaults to `NO_CONTENT` (events suppressed)
and must be set **before ADK is imported**:
...
> [!WARNING]
> **The knob only governs the standard `gen_ai.*` content events.** It does *not*
> touch ADK's vendor attributes on the spans: even at `NO_CONTENT` you will see
> `gcp.vertex.agent.llm_request` and `gcp.vertex.agent.llm_response` on the
> `call_llm` spans carrying the **full** system instruction, user message, and
> model response. If you exported the run above you already sent that text. To
> keep prompts and responses out of your backend entirely, drop these attributes
> in a span processor before export, or don't export `call_llm` spans.

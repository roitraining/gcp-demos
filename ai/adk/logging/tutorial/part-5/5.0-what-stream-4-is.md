...
Stream 4 is a separate SDK with three signals, all produced by ADK itself with
no extra packages and no instrumentation on your part:
...
answers, and 5.1 and 5.2 are those two.
...
itself. Source paths are relative to the installed `google/adk/` package
(`.venv/lib/python3.13/site-packages/google/adk/`).

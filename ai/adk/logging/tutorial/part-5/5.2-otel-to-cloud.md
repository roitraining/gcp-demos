...
You get eight entries for the one turn. Under the stable semantic convention,
which this step runs, every message is its own log: `gen_ai.system.message`,
`gen_ai.user.message` (one per item the model saw), and `gen_ai.choice` per
reply, spread across the turn's two `call_llm` spans. Expand a `gen_ai.choice`
entry.
...
Ask the London question in a **new** session, run the query again, and expand
...
Ask the London question in a new session and run the query again. Instead of
...
Ask the London question in a new session and expand the newest

...
> [!IMPORTANT]
> **What it means.** Every line is a machine-readable event. The fields
> (`latency_ms`, `status`, `input_tokens`) become keys you can filter, aggregate,
> and alert on the moment these lines reach a log store. Nothing here is
> terminal-specific, so the same records work in the cloud once a server emits
> them.

One detail the example teaches by doing. Keys in `extra=` must not collide with
built-in `LogRecord` attributes (`args`, `name`, `message`, `module`). The
example uses `tool_args`, not `args`, because `args` raises a `KeyError` inside
the log call. This bites everyone once.

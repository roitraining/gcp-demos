...
Folder: `ai/adk/metrics/` (new). Status: **Stages 0–2 done; Parts 3–4 drafted, cloud captures pending** (Stage 2 captured 2026-09-07; 2.1 read-back reworked to two-door same-chart the same day; 2.4 read-back aligned to 2.3's work-profile chart the same day). Next: Stage 3 cloud captures (3.1 done 2026-09-07; 3.2–3.7 read-backs, dashboard, alert, outcome counter remain). Sibling of `ai/adk/logging/`, same house style (`tutorial-style` skill), same project (`jwd-gcp-demos`), same model (Gemini 3.7 Flash via Vertex AI).
...
- **Q2 (scope doubling) — no doubling, but TWO scopes split by family (corrected Stage 2, 2026-09-07).** No metric is double-counted. But the earlier "one scope, `gcp.vertex.agent`" reading was incomplete: under `adk web --otel_to_cloud`, the `gen_ai.client.*` metrics (token usage, model duration) carry `otel_scope_name="opentelemetry.instrumentation.google_genai"`, while ADK's own `gen_ai.invoke_agent.*` and `gen_ai.execute_tool.*` carry `otel_scope_name="gcp.vertex.agent"`. So a scope filter must match the metric's own scope: filtering the client token metric to `gcp.vertex.agent` returns zero series. The 3.4 deep dive now teaches the two-scope split, not "defensive hygiene against doubling," and the token queries carry no scope filter. Each family still has exactly one scope, so no sum is doubled. (Not verified row 5 → verified with correction.)
...
| `examples/plot_tokens.py` | 2.1 Door 2: reads the PromQL `query_range` JSON from `out/metrics.json` and renders the two token-type lines to `out/tokens.png` (matplotlib), the same chart the Console draws |
| `queries/` | PromQL and SQL files the pages paste, one per recipe, `outcome.promql` among them; `dashboard.json`, `alert-policy.json` |
| `load/turns.sh` | `load/turns.sh <scenario> [N]`: fires N turns of one named scenario from `scenarios.md` against the running server, whatever the scenario defines: the city mix, forecast prompts, one session reused across turns, or parallel fan-out. Prints one line per turn (scenario, HTTP status, elapsed) so answered turns can be counted against exported points. After the last turn it waits `EXPORT_WAIT` seconds (default 10, one past the 5 s export interval) and prints `good to go` so the read happens after the final batch leaves the process; `EXPORT_WAIT=0` skips it. No ratio flag; the mix belongs to the scenario. Used by 2.1 onward. |
...
| `requirements.txt` | `google-adk[otel-gcp]>=2.8.0`, `opentelemetry-exporter-otlp-proto-http`, the three `opentelemetry-exporter-gcp-*` exporters, `google-cloud-aiplatform[agent-engines]`, `google-cloud-bigquery`, `google-cloud-bigquery-storage`, `google-cloud-storage`, `bigquery-agent-analytics`, `matplotlib` (2.1 chart render) |
...
| 2.1 `adk web --otel_to_cloud` | `baseline`; deep dive `export-outage` | Did the metrics land? | Three exports (`OTEL_RESOURCE_ATTRIBUTES`, service name), `load/turns.sh baseline 20` (which now waits 10 s and prints `good to go`, so the last batch has left the process before the read), confirm the series arrived by reading it **two ways, same query, same chart** (this is the first cloud read; the demo's point is "same data, different tools"): Door 1, Metrics Explorer Console with the `<> PromQL` editor — paste `sum by ("gen_ai.token.type") (rate({"...usage_sum"}[5m])) * 60`, two lines appear. Door 2, the PromQL `query_range` API — the identical query via `curl`, response written to `out/metrics.json`, opened in the editor, then `examples/plot_tokens.py` renders it to `out/tokens.png` (matplotlib) so the reader sets the API's chart beside the Console's and sees the same two lines. PromQL is declared the path Part 3 uses. How to read metrics well is Part 3. Deep dive, **What does an export outage look like?**: restart without `OTEL_RESOURCE_ATTRIBUTES`, `turns.sh baseline 5`. Every turn answers, the log shows one 400 per batch (link to logging 5.2 for the resource explanation), the read-back shows no new points, and the restart that fixes it discards the process totals. A healthy agent and complete telemetry are two different facts; count answered turns against exported points before blaming either. | both doors draw the same two-line chart; the outage window shows five answers and zero points |
...
| 2.3 `adk api_server` and Cloud Run | `baseline` (curl) | Does a deployed Cloud Run service identify itself without my help? | `adk deploy cloud_run --otel_to_cloud`, curl turns, plot the service's per-turn work profile (turns/model calls/tool calls per min) in one Metrics Explorer PromQL query filtered by `job` over the last 5 min; no `OTEL_RESOURCE_ATTRIBUTES`; `location` comes from the detector while `job` is the `--service_name` you chose. Teardown. | one chart, three lines in a 2:1:1 profile; `location` from the detector |
| 2.4 Your own server | `baseline` | Does my server export the same series as `adk web`? | `03_metrics_server.py`, `.env` works here; `enable_cloud_metrics=True`. Local run against the server's own `/chat` endpoint (a curl loop, not `turns.sh`), then read back with **the same Metrics Explorer PromQL work-profile chart as 2.3** (turns/model calls/tool calls per min via `label_replace`/`or`), filtered `job="adk-metrics"` (the server's `OTEL_SERVICE_NAME`). The curl loop prints one line per request in `turns.sh`'s format (`chat  turn N  http=200  X.XXs`). Chart described, not captured. Deep dive: the export interval. 5 s from `MIN_EXPORT_INTERVAL_MS` here and under `--otel_to_cloud`; 60 s and `OTEL_METRIC_EXPORT_INTERVAL` on the env-var route; and why a process that exits inside the interval exports nothing (`shutdown_on_exit=False`, back to 1.1). | the three-line 2:1:1 work profile under `job="adk-metrics"` |
...
| 3.1 Latency, three grains | `slow-tool`; deep dive `concurrent` | Is the dependency responsible for the slowdown? | p95 of `invoke_agent.duration` (turn), `client.operation.duration` (model), and `execute_tool.duration` (tool). **Renamed 2026-09-06** from "Latency, three ways" (collided with 2.1's three read doors); slug `3.1-latency-three-grains.md`. **Reworked 2026-09-07 (interactive):** the three read doors are gone — dropped the `timeSeries.list` Monitoring-API door and the external-`queries/` links; queries are now inlined. Page shape is Start the server → Drive the scenario → Orient in Metrics Explorer (bulleted console steps, 15-min range since there is no 5-min preset) → Read the three grains via `curl` (turn/model/tool, captured output). Step 1 starts its own `adk web --otel_to_cloud ./` server (2.1's was killed) and Step 2 `export HOST=http://localhost:8000` before `turns.sh` (a stray shell `HOST` was routing load to the wrong host → 404s). "Three grains" = turn / model / tool latency. Deep dive, **Do overlapping turns contaminate each other's timers?**: `turns.sh concurrent 20`; per-tool p95 and per-turn p95 match the sequential run because the timers are per invocation, not per process. | PromQL read-back captured 2026-09-07: turn p95 ~6.2–7.8s, model p95 ~4.8s, `get_forecast` 2.16s vs `get_weather` 0.01s |
...
| 4 | Read-back mechanism (was "Part 3 read-back") | (a) Monitoring API `timeSeries.list`; (b) PromQL HTTP endpoint via `curl`; (c) Metrics Explorer Console | **Decided: two doors on 2.1, then (b) for the rest. AMENDED 2026-09-07 — narrowed from three doors to two.** The three-doors version (Console + `timeSeries.list` + PromQL) showed three unrelated-looking outputs, which undercut the "same data, different tools" point. 2.1 now shows **two doors that run the identical PromQL query and draw the identical chart**: (c) Console PromQL editor for the visual, (b) the PromQL `query_range` API rendered to a PNG beside it. `timeSeries.list` (a) is dropped from 2.1; if the full-distribution teaching is wanted it moves to Part 3. Part 3 pages (3.1 onward) are PromQL-only via `curl`. Exception (2026-09-07): 2.3 and 2.4 Step 3 read through the Console PromQL editor (c), not `curl`, because their point is a visual work-profile chart, not a scriptable read-back. (2.4 was aligned to 2.3's chart on 2026-09-07, reversing the earlier "declined to mirror" note — see the Stage 2 entry.) PromQL addresses the dotted names in the quoted UTF-8 form (`sum by ("gen_ai.token.type")`), verified 2026-09-06. 3.1 was renamed to "Latency, three grains" (the earlier "three ways" title collided with the old 2.1 doors); **on 2026-09-07 the read doors were removed from 3.1 as well** — the `timeSeries.list` door and external-file query links are gone, leaving a Console-orient + `curl` read at three grains, consistent with the PromQL-only rule for Part 3. |
...
- **2.1 reworked (2026-09-07, interactive).** The read-back was three doors (Console described + `timeSeries.list` + PromQL scalar) whose outputs looked unrelated, so "same data, different tools" was asserted, not shown. Rebuilt to **two doors that run the identical query and produce the identical chart**: Door 1 pastes the tokens-per-type `rate(...)` query into the Console PromQL editor (two lines); Door 2 runs the same query over the `query_range` API to `out/metrics.json`, opened in the editor, then `examples/plot_tokens.py` renders it to `out/tokens.png`. Dropped the old Door 2 distribution/`count × mean` teaching (if wanted, it belongs in Part 3). New shared assets: `examples/plot_tokens.py` (matplotlib, added to requirements). Also this session: `turns.sh` now bakes in the 10 s export wait + `good to go` (was a manual "wait 10 s" step); example scripts silence three library warnings via `_common.bootstrap()` (`JSON_SCHEMA_FOR_FUNC_DECL`, the genai AFC `logging.warning` on `google_genai.models`) and `05_workflow_metrics.py` locally (`SequentialAgent` deprecation); `03_histogram_shape.py` prints a per-turn progress line; setup and 1.1 editor-open steps normalized (own `bash` fence, paren asides removed) with a tip to leave `out/metrics.json` open across runs.

- **2.3 Step 3 reworked (2026-09-07, interactive).** The read-back was a single `curl` counting turns by identity, then briefly three separate PromQL charts (turns rate, p95, tokens) which mixed units and grouped by `job` so the laptop's 2.1 series appeared as a confusing second line. Rebuilt to **one Metrics Explorer PromQL query** that unions three `_count` rates with `or` and names each with `label_replace`: turns/min, model calls/min, tool calls/min, all filtered `job="adk-metrics-otel"`. They share a unit and a Y axis and read as the 2:1:1 per-turn work profile (two model calls, one tool call per baseline turn). Corrected two facts the earlier draft had wrong: what we deploy is a Cloud Run **service**, not an "instance" (the replica is `instance`); and `job` comes from the `--service_name` flag, not the resource detector, which supplies `location` and `instance`. Chart is described, not captured; values (~10/~5/~5) are representative and consistent with the Stage 2.3 capture. Considered mirroring the query onto 2.4 and declined: 2.4 answers "does my server export the same series?" (presence, `count by (job)`), a different question, and the work profile is identical across all Part 2 baseline pages.

- **2.4 Step 3 aligned to 2.3 (2026-09-07, interactive; reverses the decline above).** On reflection the "same series as `adk web`" point is made more convincingly by reading 2.4 back with the *identical* chart 2.3 uses, so a reader sees the same three-line 2:1:1 work profile from their own server, distinguished only by the `job` label. Replaced 2.4's single `curl` `count by (job)` read with the 2.3 Metrics Explorer PromQL query (turns/model calls/tool calls per min via `label_replace`/`or`), refiltered to `job="adk-metrics"` — the server's `OTEL_SERVICE_NAME` from `env.sh` (env wins over the script's `setdefault("adk-metrics-server")`, per `verification/stage2-24-own-server.txt`). Chart described, not captured; the "What it means" now says the identical query reads back the same six series across `adk web`, Cloud Run, and the own server. Also this session: 2.4 Step 2's curl loop now prints one line per request in `turns.sh`'s format (`chat  turn N  http=200  X.XXs`) via the same `-w '%{http_code}'` + `date`/`awk` timing, since this server's `/chat` endpoint can't use `turns.sh` (which targets the `/run` + `/sessions` API); that output is illustrative.
...
- [x] **3.1 read-back captured (2026-09-07, interactive)** and the page reworked: doors removed, server-start + `HOST` steps added, output captured (turn/model/tool p95). Verification `stage3-31-latency.txt`.
- [ ] **Cloud captures (remaining):** 3.2–3.4 read-backs; 3.5 dashboard create + teardown; 3.6 alert create, fire, capture incident, teardown; 3.7 counter read-back beside 3.3's ratios.

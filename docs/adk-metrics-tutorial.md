...
| 3.1 Latency, three grains | `slow-tool`; deep dive `concurrent` | Is the dependency responsible for the slowdown? | p50/p95 of `invoke_agent.duration` and `client.operation.duration` by model; tool latency by tool. **Renamed 2026-09-06** from "Latency, three ways" (collided with 2.1's three read doors); slug `3.1-latency-three-grains.md`. Keeps the three read doors as a second pass on latency (per decision to reinforce, not remove), but the Why-note now points at 2.1 for the commit-to-PromQL step. "Three grains" = turn / model / tool latency. PromQL in `queries/`. Deep dive, **Do overlapping turns contaminate each other's timers?**: `turns.sh concurrent 20`; per-tool p95 and per-turn p95 match the sequential run because the timers are per invocation, not per process. The tightest page against the budget; the deep dive stays short. | API and PromQL read-backs agree on the percentile; concurrent per-tool p95 within tolerance of the sequential run |

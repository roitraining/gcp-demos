...
Folder: `ai/adk/tracing/` (new). Status: **plan drafted, six decisions open, nothing built** (2026-09-06). Next: Jeff settles the decisions, then Stage 0. Sibling of `ai/adk/logging/` and the planned `ai/adk/metrics/`, same house style (`tutorial-style` skill), same project (`jwd-gcp-demos`), same model (Gemini 3.7 Flash via Vertex AI).
...
**Verified so far.** Only what the logging tutorial already ran: the dev UI span tree (5.1, 2026-09-04), export to Cloud Trace and Cloud Logging under `--otel_to_cloud` (5.2), the `trace` and `spanId` fields on `gen_ai.*` log entries (5.2 captures), header-based log correlation on a custom server (4.2), and the Agent Runtime negative (Part 6). Everything else is from source and docs. The **Not verified** table under Verification is the single status board; each inline NEEDS-RUN marker names the row that resolves it.
...
| `execute_tool {name}` | `_instrumentation.py:509`, from `functions.py:709` | `invoke_agent` | `gen_ai.tool.name`, `gen_ai.tool.type` (`FunctionTool`), `gen_ai.tool.call_id`, `gen_ai.agent.name`; on failure `error.type` and `Status(ERROR, type)`, never the message (`tracing.py:232-341`); `gcp.vertex.agent.{event_id,tool_call_args,tool_response}` |
...
| Your own server | `get_gcp_exporters(enable_cloud_tracing=True)` + `maybe_set_otel_providers([hooks])`; a provider is installed only if the hooks carry a span processor | same OTLP exporter | `setup.py:45-115`, `:87` |
...
flowchart LR
  subgraph gen["one turn · spans ADK opens"]
    ...
    ag --> t1["execute_tool get_forecast"] --> cs["fetch_forecast<br/>(your span, 1.6)"]
    ag --> l2["call_llm"]
  end
...
| `tutorial/00-setup.md` | venv, `.env`, `env.sh`, APIs (`telemetry`, `cloudtrace`, `logging`), roles (`roles/telemetry.writer` to write, `roles/cloudtrace.user` and `roles/logging.viewer` to read), the demo agent |
...
| `demo_agent/` | The logging tutorial's weather agent, copied (not shared), with `get_forecast(city, days)` added: it calls a helper `_fetch_forecast()` that sleeps 0.3–1.5 s and logs one INFO line. When `TUTORIAL_CUSTOM_SPAN=1` is set, the helper runs inside `tracer.start_as_current_span("fetch_forecast")`; unset, the tree is pure ADK. Same shape as the metrics plan's `outcome.py` gate. Decision 1. |
...
| `examples/02_trace_server.py` | Minimal own server, `get_gcp_exporters(enable_cloud_tracing=True, enable_cloud_logging=True)`; returns the invocation's trace id in the `/chat` response body. The logging 5.5 shape with one flag flipped. |
| `examples/03_correlated_server.py` | 02 plus the bridge from decision 2 on the root logger. All four streams stamped. |
...
| `load/turns.sh <scenario> [N]` | Same contract as the metrics plan; prints one line per turn with the trace id when the server returns one |
| `deploy/Dockerfile.trace_server`, `deploy/env/` | Cloud Run and Agent Runtime, same pattern as logging |
| `verification/` | One run record per captured run |
| `requirements.txt` | `google-adk[otel-gcp]>=2.8.0`, `opentelemetry-exporter-otlp-proto-http`, `opentelemetry-exporter-gcp-logging`, `opentelemetry-instrumentation-fastapi`, `opentelemetry-propagator-gcp`, `google-cloud-logging` (only if decision 2 picks way B) |
...
Budgets: 200–750 words per subtask page, about 200 per landing page, one Mermaid diagram per roughly four pages (index, 1.0, 3.0, 3.4, 4.5). Prompt: "What's the weather in London?" for single turns; "What's the three-day forecast for London?" for the slow tool; `load/turns.sh <scenario> [N]` for volume.
...
| `unknown-city` | Atlantis; the tool returns an error status and does not raise | `execute_tool` carries `error.type` and status ERROR; `invoke_agent` does not; the tool's WARNING log sits under the span once 3.2 is done | Which step failed, and what did it say? |
...
| 1.4 An error turn | `unknown-city` | Which step failed? | `execute_tool get_weather` with `error.type` and `status.status_code=ERROR`; `invoke_agent` still OK. Deep dive: how `error.type` is chosen (`tracing.py:177-192`) and why the message never lands on the span. | `error.type` present on one span only |
...
| 2.3 Your own server | `baseline` | Does my server export the same tree as `adk web`? | `02_trace_server.py`: `get_gcp_exporters(enable_cloud_tracing=True, enable_cloud_logging=True)`, `.env` works here, the `/chat` response returns `trace_id`. Deep dive: no provider is installed without a span processor (`setup.py:87`); `force_flush` on shutdown. | read-back by the returned trace id |
...
| 3.2 Your tool's log line is missing | `unknown-city` | Why is the tool's warning not in the trace? | Same server: select `execute_tool get_weather`, no logs. Then `03_correlated_server.py`: the bridge on the root logger (decision 2), rerun: the WARNING sits under the failing span with `error.type` beside it. Deep dive: what the bridge captures from the record (`trace_id`, `span_id`, `trace_flags`) and what the exporter writes (`cloud_logging/__init__.py:360-367`). | the tool's log entry carries the `execute_tool` span id |
| 3.3 Framework and server logs | `slow-tool` | Do the logs I did not write land in the right spans? | Same server, `google_adk` at INFO: "Sending out request" lands under `call_llm`, the tool's helper INFO under `execute_tool` (or under `fetch_forecast` with the custom span). `uvicorn.access` has **no** trace: it runs outside any span. Deep dive: a bridge stamps whatever span is current when the record is created; records outside a span get nothing, and that is 3.4's problem. | three log names under their spans; access log without `trace` |
...
| 4.3 The failed step | `unknown-city` | Which step failed, and what did it say? | Filter **Span status** error; the red bar; `error.type` in **Attributes**; the tool's WARNING in **Logs & Events** beside it (the 3.2 payoff); the `gen_ai.choice` that told the user "no data". Deep dive: a tool that raises vs one that returns a status, and where the exception text would appear. | error span found by filter; log under it |
...
| 6 | Run the sampling page (2.5) and the unsampled-parent deep dive (3.4) for real? | (a) both captured; (b) reference only | **(a).** Both are local, cheap, and the unsampled-parent case is the one trap a reader will hit in production the week after finishing the tutorial. |
...
- [ ] Row 12: can the v1 Trace API read spans ingested through `telemetry.googleapis.com`? One `curl` against a trace id from a logging 5.2 capture.
- [ ] Row 10: does Cloud Run send `traceparent` as well as `X-Cloud-Trace-Context`? Reuse 2.2's deploy; log the headers once.
- [ ] Row 11: `ParentBased(ALWAYS_ON)` drops ADK spans under an unsampled `traceparent`; `OTEL_TRACES_SAMPLER=always_on` restores them. `01`-style script with a hand-built parent context.
- [ ] Row 3: OTel `LoggingHandler` records exported by `CloudLoggingExporter` carry `trace` and `spanId` when emitted inside a tool.
- [ ] Row 2: `force_flush()` on the tracer provider is enough for `01_console_spans.py`.

Verify: findings written into Research findings here before any page is drafted.

### Stage 1 · Scaffold and Part 1 (J)

- [ ] `README.md`, `TUTORIAL.md`, `CLAUDE.md`, `00-setup.md`, `scenarios.md`, `demo_agent/`, `requirements.txt`, `env.sh.example`, `.env.example`.
- [ ] `01_console_spans.py`.
- [ ] Pages 1.0–1.6 with captured dev UI trees and console spans.
- [ ] Rewrite and review 1.2 and 1.6 as exemplars before drafting the rest.

Verify: 1.1 shows the tool owning the time on the forecast turn; 1.3 shows `llm_request` full then `{}`; 1.6 shows the child span; every page names its scenario and question.

### Stage 2 · Part 2 collect (J; cloud writes)

- [ ] `trace/get_trace.sh`, `trace/list_traces.sh`, `load/turns.sh`.
- [ ] 2.1 local `--otel_to_cloud` with read-back; 2.2 Cloud Run deploy, read-back, teardown; `02_trace_server.py` and 2.3.
- [ ] 2.4 Agent Runtime: deploy, both views, report, teardown.
- [ ] 2.5 sampling run; 2.6 reference page.

Verify: each page's read-back block is a real capture; the 2.2 page records the two trace ids per request that 3.4 will merge.

### Stage 3 · Part 3 correlate (J; cloud writes)

- [ ] `03_correlated_server.py`, `04_propagated_server.py`.
- [ ] 3.1 with the `gcloud logging read` capture; 3.2 before-and-after; 3.3 three log names; 3.4 local `tagged-request` and `unsampled-parent`, then one Cloud Run deploy for the `request_log` join, teardown; 3.5 reference.

Verify: every log entry cited on a page carries the `spanId` of the span the page says it sits under; the unsampled-parent capture shows zero spans then a full tree.

### Stage 4 · Part 4 consume (J; cloud writes)

- [ ] 4.1 load run and the grouped-table description with a captured slowest tree; 4.2 attribute filter and search; 4.3 the error span with its log; 4.4 the scripts explained; 4.5 and 4.6 reference pages.

Verify: 4.3's captured entry is the same WARNING 3.2 captured, under the same span name.
...
|---|---|---|---|
| 0 | one `curl`, one header dump, two local scripts | rows 2, 3, 10, 11, 12 resolved in this file | not started |
| 1 | `adk web` trees; `01` locally | tool owns the forecast turn; `{}` after the knob; child span present | not started |
| 2 | `adk web --otel_to_cloud`; Cloud Run; `02`; Agent Runtime; sampled load | read-back per route; two ids per Cloud Run request recorded; sampling count within tolerance | not started |
| 3 | `02`, `03`, `04` locally; one Cloud Run deploy | entries under the right spans; unsampled parent reproduced and fixed; `request_log` joins | not started |
| 4 | load run; filters; scripts | slowest tree captured; error span with its log | not started |
...
| 2 | `force_flush()` on the tracer provider is enough for `01` | source inspection | Stage 0 |
| 3 | OTel `LoggingHandler` records carry `trace` and `spanId` through `CloudLoggingExporter` | source inspection (`cloud_logging/__init__.py:360-367`) | Stage 0 |
...
| 5 | `error.type` and ERROR status on `execute_tool` when the tool returns a status without raising | source inspection | Stage 1 (1.4) |
...
| 10 | Cloud Run sends `traceparent` in addition to `X-Cloud-Trace-Context` | proposed | Stage 0 |
...
| 12 | Trace API v1 reads OTLP-ingested spans | proposed | Stage 0 |

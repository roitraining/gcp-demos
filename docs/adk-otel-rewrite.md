...
| P3  | Readers write OTel config code that may be unnecessary        | Stages 2–4                                                                            | 5.1 through 5.4 configure telemetry with **zero** agent code (a flag, or nothing). 5.5 is the one place code is needed, and says why: you started the process, so you make the two calls the CLI would have made. Example 08 shrinks to exactly those. Human check: no OTel code appears before 5.5.                                                                                                                                                                                                |
...
| P5  | Console span output as the primary path                       | Stage 2 (cut), Stage 4 (aside)                                                        | The rewritten Part 5 has no console-exporter step in the main path. Console appears once, in a collapsed "debugging aid" aside in 5.6. Grep check: `ConsoleSpanExporter` appears at most once in `tutorial/`.                                                                                                                                                                                                                                                                                       |
...
| `ai/adk/logging/examples/08_otel_cloud.py`                     | Shrunk to the no-CLI one-liner path + GCP path; console demoted (Stage 4).                                                                                              |
| `ai/adk/logging/deploy/deploy_agent_engine.sh`                 | Comment block lines 73–74 corrected (Stage 1); no behavior change.                                                                                                      |
| `ai/adk/logging/requirements.txt`                              | Add `google-adk[otel-gcp]` extras if Stage 2 adopts them. No `OTEL_*` example vars in either env file: 5.7 shows them inline as prose, since nothing runs against them. |
| `ai/adk/logging/demo_agent/requirements.txt`                   | Already carries the container deps for `adk deploy cloud_run --otel_to_cloud` (the recorded boot-crash fix). Add the `[otel-gcp]` extra if Stage 2 adopts it (Stage 3). |
| `ai/adk/logging/deploy/deploy_otel_cloudrun.sh` (new, Stage 3) | Wraps `adk deploy cloud_run --otel_to_cloud` in the folder's deploy-script conventions (`set -euo pipefail`, `PROJECT_ID`/`REGION`, smoke test, teardown).              |
...
5.2  `adk web --otel_to_cloud`, locally, read through Cloud Logging: the required shell exports (`OTEL_RESOURCE_ATTRIBUTES` for the metrics 400, `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental`), why they must be exports not `.env`, one turn entered in the UI, the `gen_ai.*` events in Log Explorer with `<elided>` content, then the content knob (`OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT=EVENT_ONLY`) turning content on, read back in Log Explorer. ADC, APIs, roles, [otel-gcp] extras. Traces are deferred to 5.3+.
...
5.4  `adk deploy cloud_run --otel_to_cloud`: the flag rides into the container CMD; service-account roles; Cloud Run resource attributes; the recorded traps.
5.5  Your own server: when the CLI is not starting the process (Part 4's custom server, a script), you make the same two calls the CLI makes → example 08, read back in Cloud Trace. A second, non-runnable snippet shows the OTLP variant for another backend.
5.6  The two content knobs as reference (spans vs events, turning content on deliberately, RunConfig scoping); console exporter as a collapsed debugging aside
...
- Content decisions to make while writing:
  ...
  - 5.2 is `adk web --otel_to_cloud ./`, read entirely through **Cloud Logging** — traces are deferred to 5.3+ (Jeff, 2026-09-04: 5.2 looks at logs only). The flow, in order:
...
- Files: `tutorial/05-otel.md` (5.5, 5.6), `examples/08_otel_cloud.py` (`cloud` mode becomes the default and is exactly `get_gcp_exporters` + `get_gcp_resource` + `maybe_set_otel_providers`; `console` mode kept but documented only in the 5.6 aside; no `otlp` mode), `07-how-to-choose.md` references.
...
- Files: `tutorial/06-agent-runtime.md` (6.1–6.4), `deploy/deploy_agent_engine.sh` (add the `ENABLE_VIA_ENV` branch: write the env var and both knobs into the temporary `.env` and omit `--otel_to_cloud`; add the env-list read-back to the heredoc), `07-how-to-choose.md` (verification status, decision row).
- 6.1 content: the switch is the env var on the deployment (`vertexai/adk.py:1767-1815`). Two ways to write it from this folder: the flag (`cli_deploy.py:1273-1282`, also sets `ADK_CAPTURE_MESSAGE_CONTENT_IN_SPANS=false`) or a `.env` line (`cli_deploy.py:1283-1291`, sets nothing else). Also possible but not run: `agent_engines.update(config={"env_vars": …})` or the Console toggle. If you set neither, the SDK sends `unspecified` and the platform decides (Q4); the text says "do not rely on it" instead of claiming a default.
- 6.2 and 6.3: two deploys, one per route, each followed by the same three read-backs: Cloud Trace (root span name and `service.name`), Cloud Logging by ReasoningEngine resource, and the deployment's env list (`gcloud ai reasoning-engines describe … --format='yaml(spec.deploymentSpec.env)'`, ASSUMPTION on the exact field path; the vertexai client's `api_resource.spec.deployment_spec.env` is the fallback, `agents-cli` reads it that way). In 6.2 the env list shows both vars added by the CLI and the span attribute is `{}`. In 6.3 the env list shows only what `.env` carried, and because the script wrote both knobs explicitly the attribute is again `{}`; the text spells out that leaving the span knob out on this route ships prompts to Cloud Trace. Sample output = the two env lists side by side plus one span. Delete both engines at the end; teardown commands in the text.
- 6.4: only items the read-backs showed (root span `invoke_workflow weather_agent` or `invocation`, `cloud.platform`, `service.name`, log name, whether a metric appeared).
- Verify: see Verification, Stage 5.
...
- Content, all prose and one command block that is shown but not part of the run path: for CLI-launched servers the flag is replaced by env vars, honored when `--otel_to_cloud` is absent (`api_server.py:657-658`, `setup.py:124-147`); a four-row table of the vars (`OTEL_EXPORTER_OTLP_ENDPOINT` or the per-signal `_TRACES_`/`_METRICS_`/`_LOGS_ENDPOINT`, `OTEL_EXPORTER_OTLP_HEADERS`, `OTEL_SERVICE_NAME`/`OTEL_RESOURCE_ATTRIBUTES`) with the opentelemetry.io defaults; the vars must be exported in the shell, not `.env` (Q1 timing finding, cited); http/protobuf only (no gRPC exporter is imported); for your own server, the 5.5 snippet. Close with the adk.dev integrations list (`/integrations/?topic=observability`) and the note that some vendors document a code path rather than env vars (MLflow is the example: `http://localhost:5000/v1/traces` with an `x-mlflow-experiment-id` header). Everything in this section is labeled "not run here".
- Verify: see Verification, Stage 6.

### Stage 7 · 5.8 Logging interplay (J)

- Goal: one turn, seen in DEBUG logs, `LoggingPlugin`, span attributes, and `gen_ai.*` events at once, with the independent/correlated/duplicated table from Q5 and the join key.
- Problems: P8, P7.
...
- Runnable: `adk api_server --log_level DEBUG --otel_to_cloud` and the Part 3 plugin app (`examples/03_logging_plugin.py` cannot be used under `adk api_server`; ASSUMPTION: a tiny `demo_agent`-level `App(plugins=[LoggingPlugin()])` is needed, or run example 03 with the two calls from 5.5. Prefer the latter: no new agent module). Read back the `gen_ai.*` entries and the span from Cloud Logging and Cloud Trace.
- Verify: see Verification, Stage 7.

### Stage 8 · Cross-references, tables, verification status, link check (M)

- Goal: index, README, decision table, references, and nav footers agree with the new Part 5/6.
- Problems: closes P7's "explains what you are looking at" for the index diagram.
- Files: `TUTORIAL.md:67-68` (contents rows), `:39-46` (stream-4 destination label → "Cloud Trace + Cloud Logging, or any OTLP collector"), `README.md:31`, `07-how-to-choose.md` (rows 20 and 22 of the decision table, Verification status, References: add adk.dev/observability/traces|metrics|logging, the Agent Runtime tracing page, the OTel OTLP env-var page).
- Verify: `grep -rn -E "05-otel|Part 5|08_otel" ai/adk/logging` shows only intended hits; link check passes (see below).
...
| 2     | plain `adk web` + one turn (5.1); `adk web --otel_to_cloud` + one turn, then the `.env` knob line + restart + one more turn (5.2) | 5.1: Trace tab and `/debug/trace/session/{id}` show the seven expected spans with no configuration; 5.2: Cloud Trace lists an `invocation` trace within 60 s, `gcloud logging read` returns `gen_ai.user.message` with `<elided>` and a `trace` field matching the span's trace id, a `gen_ai.client.token.usage` series exists in Cloud Monitoring; the first trace's `call_llm` span carries the full prompt in `gcp.vertex.agent.llm_request` and the second trace's carries `{}`; startup WARNING text matches the doc | `otel/check_local.sh` for 5.1 (uses `api_server`, same code path) | reader sees traces before configuring anything, then finds all three signals in the console with one flag; Expected output matches the real payloads; 5.0 diagram matches the flow         | 5.2 read-backs: recorded run + date in Verification status; re-run checklist |
| 3     | `adk api_server --otel_to_cloud` + curl (5.3); `deploy/deploy_otel_cloudrun.sh`, one curl, teardown (5.4)                         | 5.3: same Cloud Trace tree as 5.2 with `llm_request` = `{}`; 5.4: service reaches Ready, one trace in Cloud Trace carries Cloud Run resource attributes and `llm_request` = `{}` (proves the `.env` knob shipped), one `gen_ai.*` entry lands on the Cloud Run resource                                                                                                                                                                                                                                                    | `bash -n` on the script; `check_local.sh` covers the shared path  | traps read as one line each and match what the deploy actually did; sample output verified                                                                                                 | the deploy: recorded run + date; teardown recorded                           |
| 4     | example 08 default (`cloud`) mode, one turn (5.5); nothing for 5.6                                                                | 5.5: Cloud Trace shows the same tree from a plain script (note: example 08 loads the folder-root `.env`, not `demo_agent/.env`, via `_common.bootstrap`, so its `llm_request` will be full unless the knob is also added to the root `.env`; record which and say so); the runnable file contains only the two GCP calls and the collapsed console aside; the OTLP snippet exists in prose only and is labeled "not run here"                                                                                              | `python examples/08_otel_cloud.py` exits 0                        | 5.5's framing sentence is the reason code appears at all, and it is the only place before 5.8; 5.6's table matches `context.py:93-113` and the deploy default at `cli_deploy.py:1281-1282` | recorded run + date                                                          |
| 5     | `deploy_agent_engine.sh` (flag, 6.2); `ENABLE_VIA_ENV=1 deploy_agent_engine.sh` (no flag, 6.3); one query each; delete both       | both: Cloud Trace shows `invoke_workflow weather_agent` (or `invocation` if the platform pins v1: record which), Cloud Logging shows OTel events on the `ReasoningEngine` resource, `llm_request` = `{}`; 6.2 env list contains `GOOGLE_CLOUD_AGENT_ENGINE_ENABLE_TELEMETRY=true` and `ADK_CAPTURE_MESSAGE_CONTENT_IN_SPANS=false` added by the CLI; 6.3 env list contains exactly the lines the script wrote and nothing the CLI added                                                                                    | `bash -n` on the script                                           | the platform-differences list contains only observed items; the two env lists in the text are the real ones; engine ids captured by the script's `ENGINE_ID=` line; both engines deleted   | recorded runs + date                                                         |
...
4. **Which exporter set runs on a native Agent Runtime deploy, ADK's or `vertexai`'s?** Determines the log name (`adk-on-agent-engine` vs `gen_ai.*`), whether the root span is `invoke_workflow`, and whether metrics appear. 6.2's read-backs show it; 6.4 states only what they showed.
...
8. ~~Example 08's filename~~ Keep `08_otel_cloud.py`. Its default mode is `cloud` and it gains no other runnable mode.

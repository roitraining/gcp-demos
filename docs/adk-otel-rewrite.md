...
- **REVISED (2026-09-05, session direction): logging-first, matching Part 5.** Read-backs are **Cloud Logging** (Logs Explorer / `gcloud logging read` on the ReasoningEngine resource) as the primary signal, plus the deployment env list. The content-knob effect is shown by **reading a `gen_ai.*` log entry's content** (`<elided>` vs text), NOT by inspecting a Cloud Trace span attribute — consistent with 5.2-5.5. Cloud Trace is mentioned as "traces also flow" in one line, not used as the read-back. NOTE for the executor: on Agent Runtime the `gen_ai.*` events may land under a different log name / resource than local (Q4, `_agent_engine.py`, `google_cloud.py:_get_agent_engine_logs_exporter`, default log name `adk-on-agent-engine`) — VERIFY LIVE what the log name and resource actually are before writing the read-back query; do not assume `gen_ai.*`.
...
- 6.1 content: the switch is the env var on the deployment (`vertexai/adk.py:1767-1815`). Two ways to write it from this folder: the flag (`cli_deploy.py:1273-1282`, also sets `ADK_CAPTURE_MESSAGE_CONTENT_IN_SPANS=false`) or a `.env` line (`cli_deploy.py:1283-1291`, sets nothing else). Also possible but not run: `agent_engines.update(config={"env_vars": …})` or the Console toggle. If you set neither, the SDK sends `unspecified` and the platform decides (Q4); the text says "do not rely on it" instead of claiming a default. This paragraph already exists (Stage 1) at `06-agent-runtime.md:16-27`; 6.1 expands it into a subsection with the two-route contrast.
- 6.2 and 6.3: two deploys, one per route, each followed by the same read-backs: **Cloud Logging** on the ReasoningEngine resource (the events, and their content field), and the deployment's env list (`gcloud ai reasoning-engines describe … --format='yaml(spec.deploymentSpec.env)'`, ASSUMPTION on the exact field path — VERIFY LIVE; the vertexai client's `api_resource.spec.deployment_spec.env` is the fallback, `agents-cli` reads it that way). In 6.2 the env list shows both vars added by the CLI and content is `<elided>` (the flag set the span knob false; whether it also sets the event knob — VERIFY). In 6.3 the env list shows only what `.env` carried; the text spells out which knobs you must set yourself on this route. Sample output = the two env lists plus one content read-back each. Delete both engines at the end; teardown commands in the text.
...
| 5     | `deploy_agent_engine.sh` (flag, 6.2); `ENABLE_VIA_ENV=1 deploy_agent_engine.sh` (no flag, 6.3); one query each; delete both       | **Logging-first (revised).** Both: **Cloud Logging** shows OTel events on the `ReasoningEngine` resource (VERIFY the log name — may be `adk-on-agent-engine`, not `gen_ai.*`, on Agent Runtime), and their content field reflects the knob; 6.2 env list contains `GOOGLE_CLOUD_AGENT_ENGINE_ENABLE_TELEMETRY=true` and `ADK_CAPTURE_MESSAGE_CONTENT_IN_SPANS=false` added by the CLI; 6.3 env list contains exactly the lines the script/`.env` wrote and nothing the CLI added. Traces "also flow" is a one-line mention, not a read-back. **NEEDS-RUN: two real deploys.** | `bash -n` on the script                                           | the platform-differences list (6.4) contains only observed items; the two env lists in the text are the real ones; engine ids captured by the script's `ENGINE_ID=` line; both engines deleted | recorded runs + date, or NEEDS-RUN blocks if deploys not run this pass       |
